...
This app owns the durable MariaDB records for the BTC straddle bot:
...
Credentials and strategy defaults are read from `Straddle Bot Settings` or
process environment variables. Sessions, orders, fills, PnL snapshots, wallet
ledger and live status are stored directly in the selected Frappe site's
MariaDB database. `--site` is mandatory.
