...
def _configure_site_runtime_defaults(frappe_module) -> dict[str, Any]:
	...
	_setenv_default("MARIADB_HOST", frappe_module.conf.get("db_host") or "127.0.0.1")
	_setenv_default("MARIADB_PORT", frappe_module.conf.get("db_port") or 3306)
	_setenv_default("MARIADB_DATABASE", frappe_module.conf.db_name)
	_setenv_default("MARIADB_USER", frappe_module.conf.get("db_user") or frappe_module.conf.db_name)
	_setenv_default("MARIADB_PASSWORD", frappe_module.conf.db_password)
...
async def _heartbeat_loop(worker_id: str, mode: str, status: str = "OK", summary: str = "Running") -> None:
	while True:
		_safe_heartbeat(
			...
			mode=mode,
			summary=summary,
			detail={"database": os.environ.get("MARIADB_DATABASE"), "storage": "MariaDB"},
		)
...
async def _run_legacy_bot(worker_id: str, runtime_mode: str) -> None:
	...
	from straddle_bot.trading.commands import claim_pending_commands, complete_command

	async def _consume_commands():
		while True:
			rows = claim_pending_commands(worker_id=worker_id, target="straddle", limit=10)
			# Commands targeted at all are included by claim_pending_commands.
			for row in rows:
				try:
					result = await straddle_trader.handle_runtime_command(row.get("command"))
					complete_command(row["name"], "Completed", "Runtime acknowledged command", result)
				except Exception as exc:
	...
	command_task = asyncio.create_task(_consume_commands())
	try:
