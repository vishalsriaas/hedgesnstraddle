...
Reads the Frappe site's MariaDB tables written by straddle_trader.py.
"""
import argparse
import asyncio
import datetime
...
DEFAULT_DB = os.environ.get("MARIADB_DATABASE", "")
DEFAULT_PORT = 8080
# Dynamic capital

def _get_capital(db) -> float:
    try:
        row = db.execute("SELECT value FROM config WHERE key='PAPER_WALLET_USDT'").fetchone()
...
# DB helpers
def _get_db() -> dbapi.Connection:
    db = dbapi.connect(_db_path, check_same_thread=False, timeout=10)   # wait up to 5s if bot is writing  # safe + fast with WAL
...
_btc_price_cache = {"price": 0.0, "ts": 0}
_btc_spot_cache = {"price": 0.0, "ts": 0}

def _fetch_btc_price() -> float:
    """Futures mark price fallback when bot is offline."""
    import time
    if time.time() - _btc_price_cache["ts"] < 3.0:
        return _btc_price_cache["price"]
    try:
        with urllib.request.urlopen(
            "https://fapi.binance.com/fapi/v1/ticker/price?symbol=BTCUSDT", timeout=0.8
        ) as r:
            px = float(json.loads(r.read())["price"])
            _btc_price_cache["price"] = px
            _btc_price_cache["ts"] = time.time()
            return px
    except Exception:
        return _btc_price_cache["price"]
...
def _fetch_btc_spot() -> float:
    ...
    import time
    if time.time() - _btc_spot_cache["ts"] < 3.0:
...
async def api_live():
    try:
        ...
        _now_ist_text = datetime.datetime.now(_IST).strftime("%Y-%m-%d %H:%M")
        expired_positions = _rows(db.execute(
            """SELECT id,expiry_dt,call_sym,put_sym,call_qty,put_qty,long_qty,short_qty
               FROM sessions WHERE state='ACTIVE' AND expiry_dt<>'' AND expiry_dt<?""",
            (_now_ist_text,),
        ))
        sess = _one(db.execute(
            """SELECT * FROM sessions
               WHERE state='ACTIVE' AND (expiry_dt='' OR expiry_dt>=?)
        ...
        _saved_call_open = float((sess or {}).get("call_fill") or 0) > 0
        _saved_put_open = float((sess or {}).get("put_fill") or 0) > 0
        _live_options_valid = bool(
            _live_position
            and (not _saved_call_open or float(_live_position.get("call_mark") or 0) > 0)
            and (not _saved_put_open or float(_live_position.get("put_mark") or 0) > 0)
        )
        _live_futures_valid = bool(
            not sess
            or (float((sess or {}).get("long_entry") or 0) <= 0
                and float((sess or {}).get("short_entry") or 0) <= 0)
            or float(_live_position.get("btc_mark") or 0) > 0
        )
        if _live_options_valid and _live_futures_valid:
        ...
        total_cap = _get_capital(db)
        roi_pct: float = (realised_pnl + unrealised_pnl) / total_cap * 100
