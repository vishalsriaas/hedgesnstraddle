...
async def get_system_health():
    ...
    try:
        if store._db_path:
            ...
            with dbapi.connect(store._db_path) as c:
                c.execute("SELECT 1 FROM paper_trades LIMIT 1")
            db_ok  = True
            db_msg = "MariaDB connected & writable"
    except Exception as e:
        db_msg = f"Error: {e}"
...
async def get_positions():
    import backend.main as m
    from backend.execution.paper_engine import is_option_symbol_expired

    def _safe_status(executor):
        status = executor.get_status() if executor else {}
        symbol = str(status.get("option_symbol") or status.get("hedge_symbol") or "")
        expired = []
        if symbol and is_option_symbol_expired(symbol):
            expired.append({
                "symbol": symbol,
                "qty": float(status.get("hedge_qty") or 0),
                "source": "api_defensive_filter",
            })
            # Do not advertise an expired contract as a running option position.
            status = dict(status)
            status["option_symbol"] = ""
            status["hedge_symbol"] = ""
            status["hedge_qty"] = 0.0
            status["hedge_unrealized_pnl"] = 0.0
        status["expired_positions"] = expired
        return status

    return {
        "bullish":  _safe_status(m.bullish) if hasattr(m, "bullish") else {},
        "bearish":  _safe_status(m.bearish) if hasattr(m, "bearish") else {},
        "ts_ist":   ist_now_str(),
    }
...
async def get_trader_trades(trader: str, limit: int = 200):
    """Persistent trades from MariaDB (survive restarts)."""
...
async def get_trades(limit: int = 200, executor: str = ""):
    """
    Returns trades from persistent MariaDB paper_trades table (survives restarts).
