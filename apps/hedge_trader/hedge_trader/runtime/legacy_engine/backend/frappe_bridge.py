...
The legacy engine still writes to its MariaDB state store. When it is launched
...
class FrappeBridge:
    ...
    async def mirror_position(self, pos_data: dict):
        frappe = self._ready()
        if frappe is None:
            return False
        try:
            from hedge_trader.trading.ingest import upsert_position, close_position
            if pos_data.get("status") == "Closed":
                return close_position(
                    position_key=pos_data["position_key"],
                    realized_pnl=pos_data.get("realized_pnl"),
                    snapshot=pos_data
                )
            return upsert_position(pos_data)
        except Exception as exc:
            log.warning("Frappe position mirror failed: %s", exc)
            return False
