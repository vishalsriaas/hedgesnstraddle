...
_exact_mark_cache: Dict[str, tuple] = {}  # symbol -> (mark, iv, fetched_epoch)
    ...
    while _running:
        ...
        try:
            ...
            if isinstance(data, list):
                ...
                target_prefix = f"BTC-{_expiry_6d}-"
                new_sym_found = any(
                    m.get("symbol", "").startswith(target_prefix) and
                ...
                for m in data:
                    sym = m.get("symbol")
                    # Keep an exact-symbol cache across expiry-chain rotations.
                    # Restored open positions must never borrow a similarly named
                    # strike from the newly selected expiry.
                    if sym and sym.startswith("BTC-"):
                        exact_mark = _safe(m.get("markPrice"))
                        exact_iv = _safe(m.get("markIV"))
                        if exact_mark > 0:
                            _exact_mark_cache[sym] = (exact_mark, exact_iv, fetched_at)
                    if sym in _sym_meta:
                        mark = _safe(m.get("markPrice"))
                        iv = _safe(m.get("markIV"))
                        new_cache[sym] = (mark, iv)
                        option = _chain.get(sym)
                        if option is not None:
                            spot = _get_spot()
                            intrinsic = _intrinsic(option["strike"], spot, option["side"])
                            option.update({
                                "mark": round(mark, 2),
                                "premium": round(mark, 2),
                                "iv": round(iv, 6),
                                "intrinsic": round(intrinsic, 2),
                                "time_value": round(max(mark - intrinsic, 0.0), 2),
                                "spot": round(spot, 2),
                                "source": "REST_MARK",
                                "ts": time.time(),
                            })
...
def get_exact_mark(symbol: str, max_age_s: float = 15.0) -> float:
    """Return a fresh mark for this exact contract symbol, never another expiry."""
    if not symbol:
        return 0.0
    live = _chain.get(symbol, {})
    live_mark = _safe(live.get("mark"))
    live_ts = _safe(live.get("ts"))
    if live_mark > 0 and live_ts > 0 and time.time() - live_ts <= max_age_s:
        return live_mark
    cached = _exact_mark_cache.get(symbol)
    if not cached:
        return 0.0
    mark, _iv, fetched_at = cached
    return float(mark) if mark > 0 and time.time() - fetched_at <= max_age_s else 0.0
