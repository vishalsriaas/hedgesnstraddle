            ...
            try:
                ...
                if self.state == "RECOVERY" and self.active_session_id:
                    ...
                    if sess:
                        ...
                        if current_recovery_val >= (0.65 * entry_premium):
                            sess.status = "Completed"
                            sess.futures_status = "Closed"
                            sess.exit_reason = "Recovery Target Hit (>= 65%)"
                ...
                if now_rel >= sq_end_rel and self.state in ["IN_TRADE", "SQUAREOFF", "ENTRY_WINDOW", "LIMITS_PLACED", "RECOVERY"]:
                    ...
                    if self.active_session_id:
                        ...
                        if sess:
                            sess.status = "Completed"
                            sess.futures_status = "Closed"
                            # Simulated recovery: close options back at current asks for bought strikes
                            rec_call = self.active_call_mark if self.active_call_mark > 0 else self.current_call_mark
                            rec_put  = self.active_put_mark if self.active_put_mark > 0 else self.current_put_mark
                            recovery_val = (rec_call + rec_put) * float(cfg.get("TRADE_QTY", "10"))
                            entry_cost = (sess.net_straddle_ask or self.combined_premium) * float(cfg.get("TRADE_QTY", "10"))
                            sess.pnl_realized = recovery_val - entry_cost
