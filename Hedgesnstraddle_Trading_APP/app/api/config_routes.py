...
def update_hedge_strategy_rules(
    ...
    strategy = db.query(HedgeStrategyConfig).filter(HedgeStrategyConfig.id == strat_id).first() if strat_id else None

    if not strategy:
        strategy = HedgeStrategyConfig(strategy_name=payload.get("strategy_name", "Bullish Hedge"))
        db.add(strategy)
