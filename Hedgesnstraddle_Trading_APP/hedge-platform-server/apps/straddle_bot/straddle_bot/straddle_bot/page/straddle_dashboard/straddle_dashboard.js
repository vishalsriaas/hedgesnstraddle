frappe.pages["straddle-dashboard"].on_page_load = function (wrapper) {
	const page = frappe.ui.make_app_page({
		parent: wrapper,
		title: __("Straddle Dashboard"),
		single_column: true,
	});
	...
	const frame = $(`
		<div class="runtime-panel-shell">
			<div class="runtime-panel-toolbar">
				<div>
					<div class="runtime-panel-title">${__("Straddle Bot")}</div>
			...
			<iframe class="runtime-panel-frame" title="${__("Straddle Dashboard")}"></iframe>
		</div>
	`).appendTo(root);
	...
	const panel_url = () => {
		if (window.location.port !== "8000") {
			return `${window.location.origin}${state.proxyPath}`;
		}
	...
	page.add_action_item(__("Straddle Bot Workspace"), () => frappe.set_route("workspace", "Straddle Bot"));
