...
MIN_EXPIRY_HOURS   = _env_float("STRADDLE_MIN_EXPIRY_HOURS", 6.0)
MIN_STRIKE_GAP     = _env_float("STRADDLE_MIN_STRIKE_GAP", 0)
#                              guarantees minimum "locked" intrinsic between strikes
MAX_PREMIUM_GAP    = _env_float("STRADDLE_MAX_PREMIUM_GAP", 100)
...
def _db_init():
    ...
    -- Per-fill record with slippage tracking (ask_at_order vs fill_price)
    CREATE TABLE IF NOT EXISTS fills (
        id            INTEGER PRIMARY KEY AUTOINCREMENT,
        session_id    INTEGER REFERENCES sessions(id),
        ts            TEXT,
        instrument    TEXT,       -- CALL_OPT | PUT_OPT | LONG_FUT | SHORT_FUT | PROT_CALL | PROT_PUT
        side          TEXT,       -- BUY | SELL
        qty           REAL,
        ask_at_order  REAL,       -- best ask when order was placed (slippage baseline)
        fill_price    REAL,       -- actual fill price
        slippage      REAL,       -- fill_price - ask_at_order
        order_id      TEXT,
        note          TEXT
    );
...
def _price() -> float:
    return _fut_px.get("mark") or 0.0
    ...
    while True:
        ...
        for oid, o in list(_paper_orders.items()):
            ...
            if asset == "future":
                ...
                sym = o.get("symbol", "")
                option_mark = _chain.get(sym, {}).get("mark", 0.0)
                lpx = o.get("price", 0)
...
class StraddleBot:
    ...
    async def _tick_manage(self, h: int, m: int):
        ...
        if not futures_open and now_min >= _to_min(*SQUAREOFF_START):
            call_mark = float(_chain.get(self.call_sym, {}).get("mark") or 0)
            put_mark = float(_chain.get(self.put_sym, {}).get("mark") or 0)
            ...
            try:
                ...
                if q.get("status") == "FILLED":
                    ...
                    if self.short_oid and not self.short_filled:
                        ...
                        _db_order_cancel(self._session_id, "SHORT_ENTRY", "OPPOSITE_FILLED")
                    if self.short_tp_oid and not self.short_tp_done:
                        await futures_cancel(self.short_tp_oid)
                        _db_order_cancel(self._session_id, "SHORT_TP", "OPPOSITE_FILLED")
                    self._entry_orders_cancelled = True
            ...
            try:
                ...
                if q.get("status") == "FILLED":
                    ...
                    if self.long_oid and not self.long_filled:
                        ...
                        _db_order_cancel(self._session_id, "LONG_ENTRY", "OPPOSITE_FILLED")
                    if self.long_tp_oid and not self.long_tp_done:
                        await futures_cancel(self.long_tp_oid)
                        _db_order_cancel(self._session_id, "LONG_TP", "OPPOSITE_FILLED")
                    self._entry_orders_cancelled = True
                ...
                if q.get("status") == "FILLED":
                    ...
                    if _db and self._session_id:
                        _db.execute(
                            ...
                            (self.long_sq_exit, _ts(), self._session_id))
                        _db.commit()
                    _db_session_update(self._session_id, sq_long_exit=self.long_sq_exit)
                ...
                if q.get("status") == "FILLED":
                    ...
                    if _db and self._session_id:
                        _db.execute(
                            ...
                            (self.short_sq_exit, _ts(), self._session_id))
                        _db.commit()
                    _db_session_update(self._session_id, sq_short_exit=self.short_sq_exit)
    ...
    async def _do_squareoff(self):
        ...
        for sym, is_open in ((self.call_sym, self.call_open), (self.put_sym, self.put_open)):
            if is_open and sym:
                option_mark = float(_chain.get(sym, {}).get("mark", 0.0)
                                    or _last_valid_opt.get(sym, 0.0) or 0.0)
                if option_mark <= 0:
        ...
        try:
            ...
            for sym, strike, side, _open_attr, _exit_attr in [
                ...
                mark = float(_chain.get(sym or "", {}).get("mark", 0.0)
                             or _last_valid_opt.get(sym or "", 0.0) or 0.0)
                if mark > 0:
    ...
    def _build_status_detail(self) -> dict:
        ...
        if self.state == "MANAGING" and self.call_fill > 0:
            _sp = _price_safe()
            _cm = _chain_get(self.call_sym,  "mark") or 0.0
            _pm = _chain_get(self.put_sym,   "mark") or 0.0
            if _cm > 0 or _pm > 0:
